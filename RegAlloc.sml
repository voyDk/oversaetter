(* Register allocator for subset of MIPS instruction set *)
...
and destReg i = (* variables and registers that can be overwritten *)
  ...
  | JALR (rs,argRegs) => addset "31" argRegs
...
fun usedRegs i = (* variables and register that can be read by i *)
  ...
  | JALR (rs,argRegs) => addsetL (rs::argRegs) emptyset
		(* r is jump register, argRegs are argumentregisters *)
...
fun conflicts [] [] callerSaves r =
  ...
  | conflicts (JALR (rs,argRegs) :: ilist) (l :: llist) callerSaves r =
      if (member r l) (* as JAL *)
      then union (remove r callerSaves)
		 (conflicts ilist llist callerSaves r)
      else if member r callerSaves
      then union (remove r l)
...
fun renameReg allocs inst =
    ...
    | JALR (rs,argRegs) =>
        [JALR (lookUp rs allocs, List.map (fn r=>lookUp r allocs) argRegs),
	 COMMENT ("was:\tjalr\t"^rs^", "^String.concat argRegs)]
    | JR (r, resRegs) =>
